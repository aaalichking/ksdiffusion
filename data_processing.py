...
class KinaseDataProcessor:
   ...
   def prepare_datasets(self):
        ...
        # 2. 统一加载全局负样本池 (只加载一次)
        hard_neg_pool, easy_neg_pool = self.load_negative_samples(exclude_peptides=all_peptides)

        # 3. 全局彻底打乱负样本池，确保后续切片的随机性
        if not hard_neg_pool.empty:
            hard_neg_pool = hard_neg_pool.sample(frac=1.0, random_state=self.config.seed).reset_index(drop=True)
        if not easy_neg_pool.empty:
            easy_neg_pool = easy_neg_pool.sample(frac=1.0, random_state=self.config.seed).reset_index(drop=True)
        ...
        # 5. 定义切片分配函数 (绝不重叠)
        def allocate_negatives(pool, n_train, n_val, n_test):
            if pool.empty:
                return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
            # 按顺序切片，确保 train/val/test 互斥
            train_neg = pool.iloc[:n_train]
            val_neg = pool.iloc[n_train : n_train + n_val]
            test_neg = pool.iloc[n_train + n_val : n_train + n_val + n_test]
            return train_neg, val_neg, test_neg

        # 分配 Hard Negatives
        train_hard_neg, val_hard_neg, test_hard_neg = allocate_negatives(
            hard_neg_pool, 
            num_train_neg // 2, 
            num_val_neg // 2, 
            num_test_neg // 2
        ...
        # 分配 Easy Negatives
        train_easy_neg, val_easy_neg, test_easy_neg = allocate_negatives(
            easy_neg_pool, 
            num_train_neg - (num_train_neg // 2), 
            num_val_neg - (num_val_neg // 2), 
            num_test_neg - (num_test_neg // 2)
        )

        # 6. 组合数据并打标签的辅助函数
        def combine_and_label(pos, hard, easy):
            neg = pd.concat([hard, easy])
            if not neg.empty:
                neg['label'] = 0.0
            pos['label'] = 1.0
            df = pd.concat([pos, neg]) if not neg.empty else pos
            # 确保标签是浮点数
            df['label'] = pd.to_numeric(df['label'], errors='coerce').fillna(0).astype(float)
            return df

        # 生成最终的训练、验证、测试集
        train_df = combine_and_label(train_pos, train_hard_neg, train_easy_neg)
        val_df = combine_and_label(val_pos, val_hard_neg, val_easy_neg)
        test_df = combine_and_label(test_pos, test_hard_neg, test_easy_neg)

        print(f"Final train size: {len(train_df)} (Pos: {len(train_pos)}, Neg: {len(train_df)-len(train_pos)})")
        print(f"Final val size: {len(val_df)} (Pos: {len(val_pos)}, Neg: {len(val_df)-len(val_pos)})")
        print(f"Final test size: {len(test_df)} (Pos: {len(test_pos)}, Neg: {len(test_df)-len(test_pos)})")
